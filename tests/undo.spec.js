...
test.describe('Undo Feature', () => {
  ...
  test.beforeEach(async () => {
    ...
    await extensionPage.goto(`chrome-extension://${extensionId}/fullpage/fullpage.html`);
  ...
  test('create folder and undo should remove it', async () => {
    ...
    const newFolderBtn = extensionPage.locator('#pane-1 button[data-action="new-folder"]');
    ...
    const testFolder = extensionPage.locator(`.content-item:has-text("${testFolderName}")`);
  ...
  test('create bookmark and undo should remove it', async () => {
    ...
    await extensionPage.locator('#pane-1 .pane-content').click({ button: 'right' });
    ...
    await extensionPage.waitForSelector(`.content-item:has-text("${testTitle}")`);
    ...
    const testBookmark = extensionPage.locator(`.content-item:has-text("${testTitle}")`);
  ...
  test('Ctrl+Z keyboard shortcut should trigger undo', async () => {
    ...
    await extensionPage.waitForSelector(`.content-item:has-text("${testFolderName}")`);
  ...
  test('rename item and undo should restore original name', async () => {
    ...
    // Wait for folder to appear
    const folderItem = extensionPage.locator(`.content-item:has-text("${originalName}")`);
    ...
    // Verify renamed
    const renamedItem = extensionPage.locator(`.content-item:has-text("${newName}")`);
  ...
  test('delete item and undo should restore it', async () => {
    ...
    // Wait for folder
    const folderItem = extensionPage.locator(`.content-item:has-text("${folderName}")`);
    await expect(folderItem).toBeVisible();
